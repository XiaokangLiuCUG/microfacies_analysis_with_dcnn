...
WIDTH = 224
epochs = 40
each_epoch_step = int(24661/BATCH_SIZE+1)
...
TRAIN_FILE = "D:/netslogs/resnet_v1_152/save_model/"
VALIDATION_FILE = "D:/netslogs/resnet_v1_152/validation_log/"
...
CKPT_FILE = "./resnet_v1_152.ckpt"

CHECKPOINT_EXCLUDE_SCOPES = 'resnet_v1_152/logits'
TRAINABLE_SCOPES = 'resnet_v1_152/block3,resnet_v1_152/block4,resnet_v1_152/logits'
...
def main():
    ...
    with slim.arg_scope(resnet_v1.resnet_arg_scope()):
        logits, _ = resnet_v1.resnet_v1_152(images, num_classes=N_CLASSES, is_training=is_training)
    ...
    with tf.control_dependencies(update_ops):
        ...
        train_step = tf.train.AdamOptimizer(learning_rate).minimize(loss,global_step=global_step,var_list=trainable_variables)
    ...
    with tf.Session() as sess:
        ...
        try:
            for epoch in range(start, epochs):
                for step in range(0,each_epoch_step):
                    ...
                    _= sess.run(train_step,feed_dict={images: tra_images,labels: tra_labels,is_training:True})
                    if (global_steps+1) % 500 == 0 or (global_steps+1) == MAX_STEPS:
                        ...
                        summary_str,train_loss,train_accuracy,train_accuracy_top3 = sess.run([summary_op,loss,evaluation_step,evaluation_step_top3],
                                                                               feed_dict={images: tra_images,labels: tra_labels,is_training:False})
                    ...
                    if (global_steps+1) % 500 == 0 or (global_steps+1) == MAX_STEPS:
                        ...
                        summary_str,validation_loss,validation_accuracy,validation_accuracy_top3 = sess.run([summary_op,loss,evaluation_step,evaluation_step_top3],
                                                                                                feed_dict={images: val_images,labels: val_labels,is_training:False})
                ...
                if (epoch+1) % 2 == 0 or (epoch+1) == epochs:
                    ...
                    for each_bach in range(num_batch):
                        ...
                        batch_correct,batch_correct_top_3 = sess.run([correct_num,correct_num_top3],
                                                                     feed_dict={images: tes_images,labels: tes_labels,is_training:False})
        ...
        #save as pb form
        '''
        constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def, ["resnet_v1_152/logits/predictions:0"])
